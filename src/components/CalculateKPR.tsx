...
function CalculateKPR({ predictionPrice }: { predictionPrice: number }) {
  ...
  useEffect(() => {
    ...
    predictionPrice,
    calcAttr.downPayment,
    calcAttr.interestRate,
    calcAttr.loanTenor,
  ]);

  // Fungsi untuk merender slider dengan angka di bawah
  const renderSliderWithLabels = ({
    label,
    maxLabel,
    minLabel,
    stepLabel,
    ...
    return (
      <div className="flex flex-col gap-2">
        <div className="flex justify-between items-center">
          <h4>{label}</h4>
          <div className="flex items-center rounded-md border border-input bg-background has-[:focus-visible]:ring-1 has-[:focus-visible]:ring-ring has-[:focus-visible]:ring-offset-2">
            <Input
        ...
        <div className="mt-4 p-4 bg-muted rounded-md">
          ...
          <div className="flex justify-between mb-2">
            ...
            <span className="font-medium">
              {formatIDR(predictionPrice * (calcAttr.downPayment / 100))}
            </span>
          </div>
          <div className="flex justify-between mb-2">
            <span>Jumlah Pinjaman:</span>
            <span className="font-medium">
              {formatIDR(calcAttr.loanAmount)}
            </span>
          </div>
          <div className="flex justify-between font-bold text-lg mt-3 pt-3 border-t">
            <span>Angsuran Bulanan:</span>
            <span className="text-primary">
              {formatIDR(calcAttr.monthlyInstallments)}
            </span>
          </div>
        </div>
      </div>
